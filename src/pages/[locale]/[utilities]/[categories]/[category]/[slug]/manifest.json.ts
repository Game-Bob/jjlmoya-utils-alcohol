...
import { ALL_TOOLS, alcoholCategory } from "../../../../../../index";
...
const loadCategories = async () => Object.fromEntries(await Promise.all(LANGUAGE_CODES.map(async (language) => [language, await alcoholCategory.i18n[language]!()]))) as Record<Language, CategoryLocaleContent>;
...
const createManifestPath = (locale: Exclude<Language, "es">, category: CategoryLocaleContent, content: ToolLocaleContent, englishContent: ToolLocaleContent) => ({
    params: { locale, utilities: getUtilityNamespace(locale), categories: getCategoryNamespace(locale), category: category.slug, slug: content.slug },
    props: { title: content.title, description: content.description, startUrl: getUtilityPath(locale, category.slug, content.slug), englishSlug: englishContent.slug },
});
